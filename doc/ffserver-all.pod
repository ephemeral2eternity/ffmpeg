...
Set the output video width expression. Default value is C<iw>. See
below for the list of accepted constants.


=item B<height, h>

Set the output video height expression. Default value is C<ih>.
...
path to the file used to read the transforms (default: F<transforms.trf>)
...
camera stays stationary. Use also C<tripod> option of
vidstabdetect.
...
virtual tripod mode: equivalent to C<relative=0:smoothing=0>
